...
export default function CreatorDetailPage() {
  ...
  const fetchChartData = async (year: string, month: string) => {
    try {
      ...
      const response = await fetch('/api/awards/creator-award/creator-award-chart', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchCreatorDetail = async () => {
      try {
        ...
        const response = await fetch('/api/awards/creator-award/creator-award-detail', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const topItems = info.list || info.top_items || []

  const totalViewVideo = getTotal(info.views)
  const totalWatchlist = getTotal(info.watchlist ?? info.watch)
  const totalWatching = getTotal(info.watch ?? info.watched)
  const totalFavorit = getTotal(info.favorits ?? info.favorit)
  ...
  {paginationHtml && (
    ...
    />
        ...
        <div className="mb-10">
          ...
          <div className="w-full overflow-x-auto rounded-xl border border-gray-800 bg-[#0b1221]">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead>
                <tr className="text-xs text-gray-500 border-b border-gray-800 bg-[#0f172a]/50">
                  ...
                  topItems.map((clip, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-300">
                        {clip.duration ?? clip.run_time_format ?? '-'}
                      </td>
                      <td className="px-4 py-3 text-gray-300">{clip.watch ?? '0'}</td>
                      <td className="px-4 py-3 text-gray-300">{clip.likes ?? '0'}</td>
                      <td className="px-4 py-3 text-gray-300">{clip.vote ?? '0'}</td>
                      <td className="px-4 py-3 text-gray-300">{clip.views ?? '0'}</td>
                      <td className="px-4 py-3 text-gray-300">{clip.play ?? '0'}</td>
                      <td className="px-4 py-3 text-gray-300">{clip.rate ?? '0'}</td>
                      <td className="px-4 py-3 text-gray-300">{clip.cats || '-'}</td>
                    </tr>
                  ))
                ) : (
                  <tr>

...
import { Heart, Plus, Share2, Play, MessageCircle, ChevronDown, ChevronUp, Music, Bookmark } from 'lucide-react'
  ...
  return (
    <div className="h-screen bg-[#020817] text-white flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex flex-1 h-[calc(100vh-80px)]">
        ...
        <aside className="hidden lg:block w-64 bg-[#020817] border-r border-white/5 px-4 py-6 h-full overflow-y-auto custom-scrollbar flex-none">
        ...
        <main className="flex-1 h-full overflow-y-scroll snap-y snap-mandatory scroll-smooth no-scrollbar relative">
          ...
          {mockClips.map((clip, index) => (
            ...
            // WRAPPER PER CLIP
            <div key={clip.id} className="w-full h-full snap-start flex items-center justify-center lg:p-8 relative">
                
                {/* Navigasi Kecil Desktop (Optional) */}
                <div className="hidden lg:flex absolute right-8 top-1/2 -translate-y-1/2 flex-col gap-96 pointer-events-none opacity-20">
                ...
                <div className="hidden lg:grid grid-cols-12 gap-8 items-center w-full max-w-6xl h-full">
                  ...
                  <div className="col-span-6 h-full flex items-center justify-center px-4 py-4">
                    <div className="relative w-full h-full max-h-[85vh] aspect-[9/16] rounded-3xl overflow-hidden border border-white/10 shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] group">
                       <Image src={clip.image} alt={clip.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
                      <div className="absolute inset-0 bg-black/20 group-hover:bg-black/30 transition-all flex items-center justify-center cursor-pointer">

...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch('https://api.usky.ai/award/watchlist', {
      method: 'POST',
      headers: {
    ...
    console.error('Awards watchlist proxy error:', error.message)
    return NextResponse.json(
      { status: false, message: 'Failed to submit watchlist' },
      { status: 500 }
    )
  }
}

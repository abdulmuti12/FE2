...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch('https://api.usky.ai/award/share', {
      method: 'POST',
      headers: {
    ...
    console.error('Awards share proxy error:', error.message)
    return NextResponse.json(
      { status: false, message: 'Failed to submit share' },
      { status: 500 }
    )
  }
}

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Number.isFinite(stars) || stars < 1 || stars > 5) {
      return NextResponse.json(
        { status: false, message: 'stars must be between 1 and 5' },
        { status: 400 }
      )
    }
    ...
    const response = await fetch('https://api.usky.ai/award/rate', {
      method: 'POST',
      headers: {
    ...
    console.error('Awards rating proxy error:', error.message)
    return NextResponse.json(
      { status: false, message: 'Failed to post award rating' },
      { status: 500 }
    )
  }
}

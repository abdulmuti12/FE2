...
export function ClipShare({ showShare, clipId, clipName, onClose, onPlatformShare }: ClipShareProps) {
      ...
      <div className="relative w-full max-w-sm bg-[#18181b] border border-white/10 rounded-2xl shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 relative group/share">
          <div className="flex items-start gap-5 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            ...
            {/* Copy */}
            <button onClick={() => onPlatformShare('copy')} className="flex flex-col items-center gap-2 min-w-[64px] flex-shrink-0 group">
            ...
            {/* WhatsApp */}
            <button onClick={() => onPlatformShare('whatsapp')} className="flex flex-col items-center gap-2 min-w-[64px] flex-shrink-0 group">
            ...
            {/* Facebook */}
            <button onClick={() => onPlatformShare('facebook')} className="flex flex-col items-center gap-2 min-w-[64px] flex-shrink-0 group">
            ...
            {/* X */}
            <button onClick={() => onPlatformShare('x')} className="flex flex-col items-center gap-2 min-w-[64px] flex-shrink-0 group">
              <div className="w-14 h-14 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-white/20 transition-colors border border-white/10">
            ...
            {/* Telegram */}
            <button onClick={() => onPlatformShare('telegram')} className="flex flex-col items-center gap-2 min-w-[64px] flex-shrink-0 group">
            ...
            {/* LinkedIn */}
            <button onClick={() => onPlatformShare('linkedin')} className="flex flex-col items-center gap-2 min-w-[64px] flex-shrink-0 group">
